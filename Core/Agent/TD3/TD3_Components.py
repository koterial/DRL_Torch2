...
class TD3_Agent():
    ...
    def get_action(self, state):
        ...
        action_batch, log_prob_batch = self.actor.get_action(state_batch, deterministic=True)
    ...
    def train(self):
        if self.replay_buffer.size() < self.batch_shape:
            return
        self.learn_step += 1
        state_batch, action_batch, _, next_state_batch, reward_batch, terminated_batch, _, index_batch, weight_batch = self.replay_buffer.sample(self.batch_shape)
...
class TD3_Actor(Deterministic_Actor):
    ...
    def train(self, state_batch):
        new_action_batch, _ = self.get_action(state_batch)
        q1_batch, _ = self.critic.get_value(state_batch, new_action_batch)
        loss = -1 * torch.mean(q1_batch)
        self.opt.zero_grad()
        loss.backward()
...
class TD3_Critic(Twin_Q_Critic):
    ...
    def train(self, state_batch, action_batch, target_q_batch, weight_batch):
        q1_batch, q2_batch = self.get_value(state_batch, action_batch)
        td_error_1_batch = target_q_batch - q1_batch
        td_error_2_batch = target_q_batch - q2_batch
        loss_1 = F.mse_loss(q1_batch, target_q_batch.detach(), reduction='none')
        loss_2 = F.mse_loss(q2_batch, target_q_batch.detach(), reduction='none')
        loss = torch.mean(weight_batch * (loss_1 + loss_2))
        self.opt.zero_grad()
        loss.backward()
        clip_by_local_norm(self.model.parameters(), self.clip_norm)
        self.opt.step()
        td_error_combined = torch.abs(td_error_1_batch) + torch.abs(td_error_2_batch)
        return loss.item(), td_error_combined / 2.0